...
try: 
    ...
    rootfrags = os.path.abspath('Schedulling.py').split("\\")
    root = ""
    for d in rootfrags:
        if d == "FieldNBalance":
            break
            ...
            root += d + "\\"
    path = os.path.join(root,"FieldNBalance","TestComponents", "TestSets", "Schedulling")

Configs = pd.read_excel(os.path.join(path, "FieldConfigs.xlsx"),skiprows=[0,1],nrows=45,usecols=lambda x: 'Unnamed' not in x)
